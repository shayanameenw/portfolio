...
const { item }: { item: { id: number; images: [string, string] } } = $props();
...
useMount(() => {
	...
	projectTimeline.to(`#service-${item.id}-img-0 > div`, {
		bottom: "100%",
	});
...
<div id={`service-${item.id}`} class="my-16 px-4 grid grid-cols-3 place-items-center">
